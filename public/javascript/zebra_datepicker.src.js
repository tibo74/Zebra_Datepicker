...
/**
 * Updated by smartcockpit SA
 * See https://github.com/tibo74/Zebra_Datepicker
 */
;(function(factory) {
	...
	$.Zebra_DatePicker = function(element, options) {
		...
		var defaults = {
			...
			// Number 1-12 based of the first month of the year, useful for school context for example, where year start in september or october.
			// It changes the way the date picker is shown for week, quarter and semester picker.
			startMonth: 1,

			needYearAdjustment: false
		};
		...
		var init = function(update) {
			...
			first_selectable_year = getFullYearWithOffset(reference_date);
			current_system_year = getFullYearWithOffset(date);
			...
			yearpicker.delegate('td:not(.dp_disabled)', 'click', function() {
				...
				if ($.inArray('months', views) > -1) {
					...
					// determine the first day of the selected semester
					var selectedDate = getFirstDateOfYear(selected_year);

					// put selected date in the element the plugin is attached to, and hide the date picker
					select_date(selectedDate.getFullYear(), selectedDate.getMonth(), 1, 'years', $(this));
				}
		...
		var generate_weekpicker = function () {
			...
			for (var i = 0; i < weekList.length; i++) {
				...
				// W + week number
				html += '<td class="' + $.trim(class_name) + '">W' + getWeekNumber(firstDateOfWeek, true)
															+ '<br/>-<br/>'
		...
		var generate_quarterpicker = function () {
			...
			for (var i = 0; i < 4; i++) {
				...
				if (is_disabled(firstDateOfQuarter.getFullYear(), firstDateOfQuarter.getMonth(), firstDateOfQuarter.getDate())) class_name += ' dp_disabled';
		...
		var generate_semesterpicker = function () {
			...
			for (var i = 0; i < 2; i++) {
				...
				if (is_disabled(firstDateOfSemester.getFullYear(), firstDateOfSemester.getMonth(), firstDateOfSemester.getDate())) class_name += ' dp_disabled';
		...
		var generate_yearpicker = function() {
			...
			for (var i = 0; i < 12; i++) {
				...
				var firstDateOfYear = getFirstDateOfYear(selected_year - 7 + i);
				if (is_disabled(firstDateOfYear.getFullYear())) class_name += ' dp_disabled';
         ...
         */
		var getWeekNumber = function(date, withOffset) {
			if (withOffset) {
				var iso8601Correction = [6, 7, 8, 9, 10, 4, 5];
				var startDateMoment = moment(0, 'HH').year(date.getFullYear()).month(plugin.settings.startMonth - 1).date(1);
				if (date < startDateMoment.toDate()) {
					startDateMoment.subtract(1, 'years');
				}

				var nds = moment(date).diff(startDateMoment, 'days') + iso8601Correction[startDateMoment.weekday()]
				var week = Math.floor(nds / 7);

				if (week === 0) {
					// Return weeknumber of dec 31st of the previous year
					return getWeekNumber(startDateMoment.subtract(-1, 'day').toDate(), withOffset);

				} else if (week === 53) {
					// Get the last day of the year
					var endOfYear = startDateMoment.add(1, 'years').subtract(1, 'day');

					// If dec 31st falls before thursday it is week 01 of next year
					if (endOfYear.weekday() < 4) {
						return 1;
					}
				}
		...
		/**
		 * Calculate the quarter number for a given date, with an optional offset of the first year's month.
		 *
		**/
		var getQuarterNumber = function (date) {
			var offset = plugin.settings.startMonth - 1;
			var monthWithOffset = date.getMonth() - offset;
			if (monthWithOffset < 0) {
				monthWithOffset += 12;
			}

			return Math.floor((monthWithOffset) / 3);
		}
		...
		**/
		var getSemesterNumber = function (date) {
			...
			return Math.floor((monthWithOffset) / 6);
		}
		/**
		...
		**/
		var getFirstDateOfWeek = function (weekNumber, month, year) {
			...
			return moment(0, 'HH').year(year).isoWeek(weekNumber).isoWeekday(1).toDate();
		}
		...
		**/
		var getFirstDateOfQuarter = function (quarterNumber, year) {
			var offset = plugin.settings.startMonth - 1;
			var monthWithOffset = quarterNumber * 3 + offset;
			if (monthWithOffset > 11) {
				year--;
				monthWithOffset += 12;
			}

			return moment().year(year).month(monthWithOffset).date(1).toDate();
		}
		...
		**/
		var getFirstDateOfSemester = function (semesterNumber, year) {
			var offset = plugin.settings.startMonth - 1;
			var monthWithOffset = semesterNumber * 6 + offset;
			if (monthWithOffset > 11) {
				year--;
				monthWithOffset += 12;
			}
		 ...
		 *
		**/
		var getFirstDateOfYear = function (year) {
			var offset = plugin.settings.startMonth - 1;

			return moment().year(year).month(offset).date(1).toDate();
		}
		...
		**/
		...
		/**
		 * Calculate the year in user system, taking into account the start month of the year.
		 * e.g: if the year starts in october, 4/18/2016 will return 2015.
		 * 
		 */
		var getFullYearWithOffset = function (date) {
			var year = date.getFullYear();

			if (plugin.settings.needYearAdjustment) {
				var offset = plugin.settings.startMonth - 1;
				var monthWithOffset = date.getMonth() - offset;
				if (monthWithOffset < 0) {
					year--;
